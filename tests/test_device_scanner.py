...
def test_list_network_devices(monkeypatch):
    ...
    sample_out = "Interface: 192.168.1.1\n  Internet Address      Physical Address\n  192.168.1.2    aa-bb-cc-dd-ee-ff"
    monkeypatch.setattr(mod.subprocess, 'check_output', lambda *a, **k: sample_out)
    ips = mod.list_network_devices()
    assert '192.168.1.2' in ips


def test_list_usb_devices_psutil(monkeypatch):
    dummy_part = types.SimpleNamespace(device='/dev/sdb1', opts='rw,removable')
    dummy_psutil = types.SimpleNamespace(disk_partitions=lambda all=False: [dummy_part])
